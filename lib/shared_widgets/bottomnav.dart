...
/// If not specified it's automatically set to [MyBottomNavigationBarType.fixed]
/// when there are less than four items, [MyBottomNavigationBarType.shifting]
/// otherwise.
///
...
class MyBottomNavigationBar extends StatefulWidget {
  ...
  /// Defines the layout and behavior of a [BottomNavigationBar].
  ///
  /// See documentation for [MyBottomNavigationBarType] for information on the meaning
  /// of different types.
    ...
    return Align(
      ...
      child: Container(
        margin: EdgeInsets.only(
          /// 这里是改变图标top值产生动态的地方，直接返回固定值
          top: type! == MyBottomNavigationBarType.fixed ? tweenStart! : Tween<double>(
            begin: tweenStart,
            end: _kTopMargin,
          ).evaluate(animation!),
//          top: Tween<double>(
...
class _BottomNavigationBarState extends State<MyBottomNavigationBar> with TickerProviderStateMixin {
  ...
  List<Widget> _createTiles() {
    ...
    if(widget.type == MyBottomNavigationBarType.fixed ) {
